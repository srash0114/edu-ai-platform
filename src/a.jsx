...
// Thêm icon ShoppingCart
import { Heart, Search, X, Star, Bot, Clock, Loader, AlertCircle, Trash2, LogOut, User, ChevronDown, MessageSquare, ShoppingCart } from 'lucide-react'; 
...
// Component: Card sản phẩm - **CẬP NHẬT**
const ProductCard = ({ product, onDetailsClick, onWishlistToggle, isWishlisted, showDelete, onDelete, onAddToCart, isInCart }) => (
    ...
    >
        ...
        <div className="p-4 flex-grow flex flex-col">
            ...
            <p className="text-sm text-gray-600 mt-1 h-10 flex-grow">{product.shortDescription}</p>
...
// Component: Modal chi tiết sản phẩm - **CẬP NHẬT**
                        ...
                        <div className="grid md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <p className="text-3xl font-extrabold text-blue-600 my-4">
                                 ...
                                 {/* UPDATED: Nút thêm vào giỏ hàng */}
                                <button 
                                    onClick={() => onAddToCart(product)}
                                    disabled={isInCart}
                                    className={`w-full py-3 rounded-lg font-bold transition-colors duration-300 flex items-center justify-center gap-2 ${isInCart ? 'bg-gray-400 cursor-not-allowed text-white' : 'bg-green-500 hover:bg-green-600 text-white'}`}
                                >
...
const SkeletonCard = () => (
    <div className="bg-white rounded-lg shadow-md overflow-hidden animate-pulse">
        ...
        <div className="p-4">
            ...
            <div className="h-10 mt-3 bg-gray-300 rounded w-full"></div>
        </div>
    </div>
);
...
// Component chính của ứng dụng - **CẬP NHẬT**
export default function App() {
    ...
    const [cart, setCart] = useState([]); // NEW: State cho giỏ hàng
    ...
    // Load wishlist, view history, và cart của user khi đăng nhập
    useEffect(() => {
        if (currentUser) {
            ...
            const savedCart = localStorage.getItem(`cart_${currentUser.email}`); // NEW
            setWishlist(savedWishlist ? JSON.parse(savedWishlist) : []);
            setViewHistory(savedHistory ? JSON.parse(savedHistory) : []);
            setCart(savedCart ? JSON.parse(savedCart) : []); // NEW
        } else {
            // Reset khi logout
            setWishlist([]);
            setViewHistory([]);
    ...
    // Lưu wishlist, lịch sử xem, và giỏ hàng vào localStorage khi thay đổi
    useEffect(() => {
        if (currentUser) {
    ...
    // NEW: Lưu giỏ hàng vào localStorage
    useEffect(() => {
        if (currentUser) {
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredProducts.map(product => (
                        <ProductCard
                            ...
                            onAddToCart={handleAddToCart} // NEW: Truyền hàm vào Card
                            isInCart={!!cart.find(item => item.id === product.id)} // NEW: Kiểm tra sản phẩm có trong giỏ không
                        />
                    ))}
                </div>
            </AnimatePresence>
    ...
    return (
        <div className="bg-gray-100 min-h-screen font-sans">
                ...
                <div className="container mx-auto px-4 py-4 flex flex-col md:flex-row justify-between items-center">
                    <div className="flex justify-between items-center w-full md:w-auto mb-4 md:mb-0">
                        <h1 className="text-2xl font-bold text-blue-600">EduAI Platform</h1>
                         {/* Cập nhật mobile header để bao gồm giỏ hàng */}
                            ...
                            <div className="relative" ref={dropdownRef}>
                                ...
                                <AnimatePresence>
                                    {showUserDropdown && (
                                        ...
                                        >
                                            <button onClick={handleProfileClick} className="w-full text-left px-4 py-2 text-gray-700 hover:bg-gray-100 flex items-center gap-2">
                                                <User size={18} /> Hồ sơ
                                            </button>
                                            <button onClick={handleLogout} className="w-full text-left px-4 py-2 text-red-600 hover:bg-gray-100 flex items-center gap-2 border-t">
            ...
            <main className="container mx-auto px-4 py-8">
             ...
             {/* NEW: Chatbot Button is now separate from the sidebar */}
            <button
